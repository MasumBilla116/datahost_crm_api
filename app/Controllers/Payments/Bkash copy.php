        ...
        try {
            ...
            if (!empty($_GET['status']) && strtolower($_GET['status']) === 'success') {
                ...
                if (!empty($paymentExecute->transactionStatus) && (strtolower($paymentExecute->transactionStatus) == "completed")) {
                    ...
                    Accounting::Accounts($roomBooking->account_id, $roomBooking->id, "booking",  $roomBooking->total_paid, $credited_note, $debited_note, $roomBooking->customer_id, true);
                    ...
                    DB::table('payment_collection_slip')->insert([
                        ...
                        "account_id" => $roomBooking->account_id,
    ...
    public function bookingCreation(Request $request, Response $response)
    {
        ...
        try {
            ...
            $customer_booking = $this->customer_booking->insertGetId([
                ...
                // 'date_from' => date('Y-m-d', strtotime("+1 day", strtotime($this->params->date_from))),
                // 'date_to' => date('Y-m-d', strtotime("+1 day", strtotime($this->params->date_to))),
                'account_id' => $this->params->account_id,
                'date_from' => date('Y-m-d', strtotime($this->params->date_from)),
                'date_to' =>  $to_date_,
                ...
                'net_amount' => $this->params->roomPrice,
                'total_paid' => 0, // @@@@ Unpaid
                'refund_amount' => 00,
            ...
            //     ->select("id")
            //     ->where(DB::raw("LOWER(account_name)"), strtolower("bkash"))
            //     ->first();


            // Accounting::Accounts($account->id, $customer_booking, "booking", $this->params->netPayable, $credited_note, $debited_note, $customerId, true);
            // Accounting::accountCustomer($credit, $customerId, $customer_booking, $invoice_id, "booking", $this->params->netPayable, $customer_booking_grp->total_paid, $credited_note, $debited_note, $customerId, false);
