...
class CustomerController
{
    ...
    public function cloneCustomerBooking(Request $request)
    {
        ...
        try {
            ...
            $rb_inv = 'RB-REF-' . $bookingId . '-' . strtotime('now');

            // Create a new stdClass instance to store the cloned booking data
            $cloneBooking = new stdClass();
            $cloneBooking->invoice_id = $rb_inv;
            $cloneBooking->promo_id = null;
            $cloneBooking->promo_discount = 0;
            $cloneBooking->additional_discount = 0;
            $cloneBooking->total_tax = 0;
            $cloneBooking->total_amount = 0;
            $cloneBooking->net_amount = 0;
            $cloneBooking->total_paid = 0;
            $cloneBooking->refund_amount = 0;
            $cloneBooking->total_due = 0;
            $cloneBooking->platform = "FrontDesk";
            $cloneBooking->invoice_type = "booking";
            $cloneBooking->status = 3;
            $cloneBooking->payment_status = 0;
            $cloneBooking->payment_type = "cash on the spot";

            $cloneBooking->customer_id = $bookingData->customer_id;
            $cloneBooking->date_from = $bookingData->date_from;
            $cloneBooking->date_to = $bookingData->date_to ?? date("Y-m-d", strtotime($bookingData->date_from));
            ...
            // @@ clone booking days
            // $cloneBookingDays = new stdClass();

            // $cloneBookingDays->booking_master_id = $cloneBookingId;
            // $cloneBookingDays->customer_id = $bookingData->customer_id;
            // $cloneBookingDays->room_id = null;
            // $cloneBookingDays->tower_id = null;
            // $cloneBookingDays->room_type_id = null;
            // $cloneBookingDays->room_category_id = null;
            // $cloneBookingDays->date = null;
            // $cloneBookingDays->adults = null;
            // $cloneBookingDays->childs = null;
            // $cloneBookingDays->tarrif_id = null;
            // $cloneBookingDays->tarrif_amount = 0;
            // $cloneBookingDays->additional_total_adult_amount = 0;
            // $cloneBookingDays->additional_total_child_amount = 0;
            // $cloneBookingDays->total_amount = 0;
            // $cloneBookingDays->status = 1;

            // DB::table('customer_booking_days')->insertGetId((array) $cloneBookingDays);


            if ($cloneBookingId) {
                DB::commit();
                $this->responseMessage = "Booking cloned successfully";
                $this->outputData =  ["booking_id" => $cloneBookingId];
                $this->success = true;
            } else {
                DB::rollBack();
    ...
    public function createCustomer(Request $request)
    {
        ...
        if ($this->params->corporatePay === false && ($payment_status === 1 || $payment_status === 2)) {
            $this->validator->validate($request, [
                "account_id" => v::notEmpty()
            ]);
        }
        ...
        try {
            ...
            if ($existcustomer && $this->params->newCustomer === false) {
                ...
                $customer->balance = $existcustomer->balance;
            ...
            if ($customer) {
                ...
                // @@ default value for paid and due amount
                $customer_booking_grp->total_paid = 0;
                $customer_booking_grp->total_due = 0;
                $customer_booking_grp->payment_status = 0;
                ...
                if (($payment_status === 1)) {
                    ...
                    $customer_booking_grp->total_due =  0;
                    $customer_booking_grp->payment_status = 1;
                    $customer_booking_grp->advance_payment = $total_paid;
                } else if ($payment_status === 2) {
                    $customer_booking_grp->total_paid = $total_paid;
                    $customer_booking_grp->total_due =  $net_amount - $total_paid;
                    $customer_booking_grp->payment_status = 2;
                    $customer_booking_grp->advance_payment = $total_paid;
                } else {
                    $customer_booking_grp->total_paid = 0;
                    $customer_booking_grp->total_due =  $net_amount;
                    $customer_booking_grp->payment_status = 3;
                    $customer_booking_grp->advance_payment = 0;
                }

                /**
                 * @@ room booking status for check-in or booked @@
                 * ------------------------------------------------
                 * 1. for check-in
                 * 2. for booked
                 * 
            ...
            if ($customer_booking_grp) {
                ...
                if ($customer->customer_type === 0 or ($customer->customer_type === 1 and $corporate_pay === false)) {
                    ...
                    if (!empty($this->params->account_id) && ($customer_booking_grp->total_paid > 0) && ($payment_status === 1 || $payment_status === 2)) {
                        $credited_note = "payment taken from customer";
                        $debited_note = "";
                    ...
                    if ($customer_booking_grp->total_paid > 0 && ($payment_status === 1 || $payment_status === 2)) {
                        ...
                        DB::table('payment_collection_slip')->insert([
                            ...
                            'reference' => $this->params->reference ?? "Reference is not yeat",
                            'remark' => $this->params->remark  ?? "Remark is not yeat",
                ...
                //Account Tax
                if ($this->params->tax_id && ($payment_status === 1 || $payment_status === 2)) {
            ...
            $this->outputData = ['customer' => $customer, 'booking' => $customer_booking_grp];
            $this->success = true;
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function updateBooking(Request $request)
    {
        ...
        try {
            ...
            if ($customer_booking_grp->payment_status == 3) {
                $old_net_amount = 0;
                $customer_booking_grp->payment_status = 0;
            } else {
                $old_net_amount = floatval($customer_booking_grp->net_amount);
            }
            //customer
            ...
            if ($this->params->date_to !== null && (strtolower($this->params->checkout_type) != 'hourly')) {
                // $customer_booking_grp->date_to = date('Y-m-d',strtotime("+1 day", strtotime($this->params->date_to)));
                $customer_booking_grp->date_to = date('Y-m-d', strtotime($this->params->date_to));
                $end = $customer_booking_grp->date_to;
            } else if (strtolower($this->params->checkout_type) == 'hourly') {

                $customer_booking_grp->date_from = $begin;
                $customer_booking_grp->date_to = $begin;
                $end =  $begin;
                $begin = date('Y-m-d', strtotime("-1 day", strtotime($this->params->date_from)));
            } else {
                $end = $begin;
            }
            ...
            // dd(date('Y-m-d', strtotime($this->params->date_from)));
            //Tarrif information for date range price info, tarrif id, tarrif amount
            ...
            $additional_discount = $this->params->additional_discount;
            $total_tax = $this->params->total_tax;
            $advance_paid = floatval($this->params->advance_paid) ?? 0;
            ...
            $customer_booking_grp->total_due = $net_amount - $advance_paid;
            $customer_booking_grp->total_paid = $advance_paid;
            ...
            if ($customer_booking_grp) {
                ...
                if ($customer->customer_type === 0 or ($customer->customer_type === 1 and $corporate_pay === false)) {
                    ...
                    */
                    $new_net_amount = floatval($this->params->net_amount);

                    $advance_payment =  floatval($customer_booking_grp->advance_payment); //21
                    $customer_balance = floatval($customer->balance); //0


                    if ($customer_balance < 0) {
                        $customer_balance = ($old_net_amount + $customer_balance); //21+0
                    } else {
                        $customer_balance = ($customer_balance + $old_net_amount); //0+21=21
                    }


                    $customer_balance = ($customer_balance - $advance_payment); //21-21=0

                    $newBalance = ($customer_balance - $new_net_amount); //0-31.5=-31.5


                    if ($newBalance < 0) {
                        $newBalance += $advance_payment;   //-31.5+21=-10.5
                    } else {
                        $newBalance += $advance_payment;
                    }
                    // dd($newBalance);
                    ...
                    DB::table('account_customer')->insert([
                        ...
                        'invoice_id' =>  $customer_booking_grp->id,
                        'inv_type' => $customer_booking_grp->invoice_type,
                        'reference' =>  $customer_booking_grp->invoice_id,
                        'debit' => - ($net_amount),
                        'credit' => 0,
                    ...
                    // Accounting::accountCustomer($credit, $customer->id, $customer_booking_grp->id, $customer_booking_grp->invoice_id, $customer_booking_grp->invoice_type, $net_amount, $customer_booking_grp->advance_payment, $credited_note, $debited_note, $this->user->id, false);
            ...
            DB::commit();
            $this->responseMessage = "Booking update successfully";
